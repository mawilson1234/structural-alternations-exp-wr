...
for (i in seq_along(model.lists)) {
	models[sprintf('Crossed model (non-linear) %02d', i)] <- do.call(brm, append(brm.args, list(
		...
		file = file.path(models.dir, sprintf('crossed_model_accuracy_non-linear_%02d.rds', i))
	))) |> list()
}

save_model_summaries(
	models,
	filename='crossed_model_accuracy_non-linear_summaries.txt', 
	overwrite=TRUE
)

save_pmcmc(
	models,
	filename='crossed_model_accuracy_non-linear_pmcmcs.txt'
)
